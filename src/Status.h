...
inline char* Status::vcs() { return prepared(); }
...
inline void Status::mark(Seqno n, char* set) {
  ...
  if (offset > 0 && offset <= max_out) Bits_set(set, offset);
}

inline bool Status::test(Seqno n, char* set) {
  int offset = n - last_executed();
  if (offset > 0 && offset <= max_out) return Bits_test(set, offset);
  return false;
}
...
inline void Status::add_breqs(Seqno n, BR_map brs) {
  th_assert(has_nv_info(), "Invalid state");
  th_assert(n > last_executed() && n <= last_stable() + max_out,
            "Invalid arguments");
