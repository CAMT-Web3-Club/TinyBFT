...
bool NV_info::add(New_view* m, View_info* parent) {
  th_assert(m->verify() || node->id() == node->primary(m->view()),
            "Invalid argument");
  ...
  for (int i = 0; i < node->n(); i++) {
    Digest vd;
    if (m->view_change(i, vd)) vc_target++;
  }
...
bool NV_info::add(View_change_ack* m) {
  ...
  if (!is_primary) {
    if (!in_nv) return false;
  } else {
    if (!vcs[vci].vc) return false;
  ...
  if (!in_nv && m->vc_digest() == d && !vcs[vci].ack_reps.test(mid)) {
    ...
    if (vcs[vci].ack_count == node->f() * 2 - 1 &&
...
void NV_info::summarize(View_change* vc) {
  ...
  for (int i = 0; i < size; i++) {
    ...
    if (cur.n == vcn && cur.d == vclc) {
      ...
      cur.n_proofs++;
      cur.n_le++;
      if (vc->max_seqno() > cur.max_seqno) cur.max_seqno = vc->max_seqno();
    } else {
      Digest d;
      ...
      if (cur.n > vcn) {
        ...
        n_le++;
        if (cur.max_seqno > max_seqno) max_seqno = cur.max_seqno;
      }
    }
...
void NV_info::choose_ckpt(int index) {
  ...
  th_assert(cur.n_proofs >= node->f() + 1 && cur.n_le >= node->n_f(),
            "Invalid argument");
  ...
  if (chosen_ckpt < 0) {
    ...
    for (Seqno i = min + 1; i <= cur.n; i++) {
      if (comp_reqs[i - base] >= 0) n_complete--;
    }
...
void NV_info::check_comp(Req_sum& cur, Seqno i, int j) {
  ...
  if (comp_reqs[i - base] < 0 && cur.n_proofs >= node->f() + 1 &&
      cur.n_le >= node->n_f()) {
...
Seqno NV_info::known_stable() {
  ...
  // max_stable2 <= min
  if (max_stable2 > min) max_stable2 = min;
...
void NV_info::get_proofs(Req_sum& cur, View_change* vc, Seqno i) {
  ...
  if (!vc->proofs(i, v, lv, d, prepared)) {
    if (i > vc->last_stable() && cur.v < 0) cur.n_proofs++;
    return;
  }
  ...
  for (i = i + 1; i < max; i++) {
    ...
    if (!match) {
      ...
      for (int j = 0; j < vcs.size(); j++) {
        if (vcs[j].req_sum) get_proofs(cur, vcs[j].vc, i);
      }
...
bool NV_info::check_new_view() {
  ...
  min = vc->last_stable();
  if (min != nv->min()) return false;
  base = min + 1;

  Digest d;
  if (!vc->ckpt(min, d)) return false;
  ...
  for (int i = 0; i < vcs.size(); i++) {
    if (i != cid && vcs[i].vc) {
      if (vcs[i].vc->ckpt(min, dd) && dd == d) n_proofs++;
      if (vcs[i].vc->last_stable() <= min) n_le++;
    }
  }

  if (n_proofs < node->f() + 1 || n_le < node->n_f()) return false;
  ...
  for (int i = 0; i < vcs.size(); i++) {
    ...
    if (vc->max_seqno() < nv->max()) n_lt++;
  }

  if (n_lt < node->n_f()) return false;
  ...
  for (Seqno i = base; i < nv->max(); i++) {
    ...
    for (int j = 0; j < vcs.size(); j++) {
      if (vcs[j].vc) {
        ...
        Digest dd;
        if (vcs[j].vc->req(i, dd) <= cur.v) cur.n_le++;
      }
    }
...
Pre_prepare* NV_info::fetch_request(Seqno n, Digest& d) {
  ...
  if (node->primary(v) == node->id()) pp->re_authenticate();
...
void NV_info::set_received_vcs(Status* m) {
  if (node->primary(v) != node->id()) {
    ...
    Digest d;
    for (int i = 0; i < node->n(); i++) {
      if (vcs[i].vc || !nv->view_change(i, d)) m->mark_vcs(i);
    }
  } else {
    for (int i = 0; i < node->n(); i++) {
      if (vcs[i].vc && nv->view_change(i)) m->mark_vcs(i);
    }
  }
}
...
void NV_info::set_missing_pps(Status* m) {
  for (Seqno i = base; i < max; i++) {
    if (comp_reqs[i - base] >= 0) continue;
    ...
    if (vpp < v) m->append_pps(vpp, i, mrmap, need_proofs);
  }
}
