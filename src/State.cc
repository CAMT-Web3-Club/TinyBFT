...
State::State(Replica* rep, int num_objs, int (*gets)(int, char**),
    ...
    : replica(rep),
      nb(num_objs + replica->used_state_pages()),
      cowb(nb),
      clog(max_out * 2, 0),
      lc(0),
      get_segment(gets),
...
#ifndef NO_STATE_TRANSLATION
char* State::get_cached_obj(int i) { return pages_lc->get_obj(i); }
#endif
...
char* State::get_data(Seqno c, int i) {
  ...
  static int last_sz, last_i = -1,
                      last_c;  // values of the object size, i
