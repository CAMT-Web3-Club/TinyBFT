...
BH_TEMPLATE BHashT::BHash(int size_hint) {
  MEMSTATS_CALL_STACK_PUSH(BHashT::BHash);
  numItems = 0;
  sizeup(size_hint);
...
BH_TEMPLATE void BHashT::copy_items(BHashT const &bh) {
  MEMSTATS_CALL_STACK_PUSH(BHashT::copy_items);
  numItems = 0;
  sizeup(bh.numItems);
  buckets = new SET[numSlots];
  BHashGenerator<ELEM, Buckets<ELEM>, BucketsGenerator<ELEM> > g((BHashT &)bh);
  ELEM e;
  while (g.get(e)) add(e);
  MEMSTATS_CALL_STACK_POP();
}

BH_TEMPLATE BHashT::BHash(BHashT const &bh) { copy_items(bh); }
...
BH_TEMPLATE int BHashT::size() const { return numItems; }
...
BH_TEMPLATE void BHashT::predict(int desired_size) {
  int ns = numSlots;
  if (ns >= desired_size &&
      ns < desired_size * BH_EXCESSIVE_RECIPROCAL_ALPHA + BH_MIN_SIZE)
    return;
