...
View_info::View_info(int ident, View vi)
    : v(vi),
      ...
      last_stable(0),
      oplog(max_out),
      last_views((View)0, node->n()),
      last_vcs((View_change *)0, node->n()),
      my_vacks((View_change_ack *)0, node->n()),
      vacks(node->n()),
      last_nvs(node->n()) {
...
bool View_info::prepare(Seqno n, Digest &d) {
  ...
  if (oplog.within_range(n)) {
    ...
    if (ri.d == d) return true;

    for (int i = 0; i < node->f() + 2; i++) {
      if (ri.ods[i].d == d) return true;
    }
  }
    ...
    for (int i = 0; i < node->n(); i++) {
      ...
      if (vaci.v == v) {
        for (int j = 0; j < node->n(); j++) {
          if (vaci.vacks[j] && n.add(vaci.vacks[j])) vaci.vacks[j] = 0;
        }
      }
    }
  }
...
New_view *View_info::my_new_view(Time **t) { return last_nvs[id].new_view(t); }
...
bool View_info::restart(FILE *in, View rv, Seqno ls, bool corrupt) {
  ...
  for (int i = 0; i < node->n(); i++) last_views[i] = v;
  ...
  ab++;
  if (is_complete) last_nvs[node->primary(v)].make_complete(v);
