...
#define ROTL32_VAR(r, n) (((r) << ((n)&31)) | ((UINT32)(r) >> (32 - ((n)&31))))
#define ROTL32_CONST(r, n) (((r) << (n)) | ((UINT32)(r) >> (32 - (n))))
...
static UINT64 MUL64(UINT32 a, UINT32 b) { return (UINT64)a * (UINT64)b; }
...
static void STORE_UINT32_REVERSED(void *ptr, UINT32 x) {
  UINT32 i = (UINT32)x;
  *(UINT32 *)ptr =
      (i >> 24) | ((i & 0x00FF0000) >> 8) | ((i & 0x0000FF00) << 8) | (i << 24);
}
...
/* Load little endian key into L */
  ...
  S[0] = RC6_P;
  for (i = 1; i < RC6_TABLE_WORDS; i++) S[i] = S[i - 1] + RC6_Q;
...
static void nh_aux_32(void *kp, void *dp, void *hp, UINT32 dlen) {
  nh_aux_16(kp, dp, hp, dlen);
  nh_aux_16((INT8 *)kp + ((8 / WORD_LEN) * L1_KEY_SHIFT), dp, (INT8 *)hp + 16,
            dlen);
}
...
#else
#define endian_convert_if_le(x, y, z) \
  do {                                \
  } while (0) /* Do nothing */
#endif
...
static UINT64 poly64(UINT64 cur, UINT64 key, UINT64 data) {
  UINT32 key_hi = (UINT32)(key >> 32), key_lo = (UINT32)key,
         cur_hi = (UINT32)(cur >> 32), cur_lo = (UINT32)cur, x_lo, x_hi;
  UINT64 X, T, res;
...
static void poly_hash(uhash_ctx_t hc, UINT32 data[]) {
  ...
  if (hc->poly_invocations < poly_crossover) { /* Use poly32 */
    for (i = 0; i < PREFIX_STREAMS; i++) {
      ...
      if (data[i] >= (p32 - 1)) {
        hc->poly_accum[i] =
            poly32((UINT32)hc->poly_accum[i], hc->poly_key_4[i], p32 - 1);
        hc->poly_accum[i] =
            poly32((UINT32)hc->poly_accum[i], hc->poly_key_4[i], (data[i] - 5));
      } else
        hc->poly_accum[i] =
            poly32((UINT32)hc->poly_accum[i], hc->poly_key_4[i], data[i]);
    }
    ...
    if (!hc->poly_store_full) {
      ...
      for (i = 0; i < PREFIX_STREAMS; i++) {
        ...
        if ((UINT32)(hc->poly_store[i] >> 32) == 0xfffffffful) {
          hc->poly_accum[i] =
              poly64(hc->poly_accum[i], hc->poly_key_8[i], p64 - 1);
...
static void ip_long(uhash_ctx_t ahc, char *res) {
  ...
  if (ahc->poly_invocations > poly_crossover) { /* hash 64 bits */
    ...
    for (i = 0; i < PREFIX_STREAMS; i++) {
      if (ahc->poly_accum[i] >= p32) ahc->poly_accum[i] -= p32;
 ...
 */
  ...
  for (i = 0; i < PREFIX_STREAMS; i++)
    memcpy(ahc->ip_keys + 4 * i, buf + 8 * sizeof(LARGE_WORD) * i,
           4 * sizeof(LARGE_WORD));
  endian_convert_if_le(ahc->ip_keys, sizeof(LARGE_WORD), sizeof(ahc->ip_keys));
 ...
 */
{
  ...
  if (ctx->msg_len + len <= L1_KEY_LEN) {
    ...
    if (ctx->msg_len == L1_KEY_LEN) bytes_hashed = L1_KEY_LEN;
 ...
 */
    ...
    if (ctx->poly_invocations > poly_crossover) {
      ...
      int i;
      for (i = 0; i < PREFIX_STREAMS; i++) tmp[i] = 0x80000000u;
      poly_hash(ctx, tmp);
...
#endif
  char pt[] =
      "\x02\x13\x24\x35\x46\x57\x68\x79"
...
static void umac_verify(void) {
  ...
  if (bytes_over_boundary != 0) data_ptr += (16 - bytes_over_boundary);
  for (i = 0; i < data_len; i++) data_ptr[i] = (i % 127) * (i % 123) % 127;
  ctx = umac_new("abcdefghijklmnopqrstuvwxyz");

  umac(ctx, data_ptr, data_len, tag, nonce);
  umac_reset(ctx);

#if ((WORD_LEN == 2) && (UMAC_OUTPUT_LEN == 8) && (L1_KEY_LEN == 1024) && \
     (UMAC_KEY_LEN == 16))
  printf("UMAC-2/8/1024/16/LITTLE/SIGNED Test\n");
  pbuf(tag, PREFIX_STREAMS * WORD_LEN, "Tag is                   ");
  printf("Tag should be a prefix of: %s\n", "F238FCB732528C51");
#elif ((WORD_LEN == 4) && (UMAC_OUTPUT_LEN == 8) && (L1_KEY_LEN == 1024) && \
       (UMAC_KEY_LEN == 16))
  printf("UMAC-2/8/1024/16/LITTLE/SIGNED Test\n");
  pbuf(tag, PREFIX_STREAMS * WORD_LEN, "Tag is                   ");
  printf("Tag should be a prefix of: %s\n", "64BA28EFC26175AB");
...
static void speed_test(void) {
  ...
  long tag_iters = (500 * 1024 * 1024) / (data_len * iters_per_tag), i, j;
  ...
  for (i = 0; i < data_len; i++) data[i] = (i * i) % 127;
  uc = umac_new(key);
