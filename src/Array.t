...
Array<T>::Array(Array const& d) {
  ...
  store_ = 0;
  enlarge_to(d.size_);
  for (int i = 0; i < size_; i++) store_[i] = d.store_[i];
}
...
void Array<T>::append(T element, int n) {
  ...
  int oldsize = size_;
  enlarge_to(size_ + n);
  for (int i = 0; i < n; i++) store_[i + oldsize] = element;
}
...
void Array<T>::concat(Array const& d) {
  ...
  for (int i = 0; i < d.size_; i++) store_[i + oldsize] = d.store_[i];
}
...
void Array<T>::concat(T const* src, int s) {
  ...
  int oldsize = size_;
  enlarge_to(s + size_);
  for (int i = 0; i < s; i++) store_[i + oldsize] = src[i];
}

template <class T>
void Array<T>::predict(int new_alloc) {
  if (new_alloc > alloc_) enlarge_allocation_to(new_alloc);
}
...
void Array<T>::enlarge_allocation_to(int newsize) {
  ...
  for (int i = 0; i < size_; i++) store_[i] = oldstore[i];
...
void Array<T>::reclaim() {
  if (alloc_ > size_) {
    ...
    if (size_ > 0) {
      ...
      for (int i = 0; i < size_; i++) newstore[i] = store_[i];
      delete[] store_;
      alloc_ = size_;
      store_ = newstore;
    } else {
