...
View_change::View_change(View v, Seqno ls, int id)
  ...
  bzero((char *)rep().prepared,
        View_change_rep::prepared_size * sizeof(unsigned));
  rep().n_reqs = 0;
...
void View_change::add_checkpoint(Seqno n, Digest &d) {
  ...
  th_assert((last_stable() <= n) && (n <= last_stable() + max_out),
            "Invalid argument");
...
bool View_change::proofs(Seqno n, View &v, View &lv, Digest &d,
                         bool &prepared) {
  int index = n - last_stable() - 1;
...
void View_change::re_authenticate(Principal *p) {
  ...
  th_assert(
      ...
      "Invalid state");
  th_assert(rep().n_ckpts == 0 || rep().ckpts[rep().n_ckpts - 1] != Digest(),
            "Invalid state");
  th_assert(last_stable() >= 0 && last_stable() % checkpoint_interval == 0,
            "Invalid state");
...
bool View_change::verify() {
  ...
  if (nckpts < 0 || nckpts > max_out / checkpoint_interval + 1) return false;

  if (nckpts > 0 && rep().ckpts[nckpts - 1].is_zero()) return false;
  ...
  if (size() - old_size < node->auth_size(id())) return false;

  // Check consistency of request information.
  for (int i = 0; i < nreqs; i++) {
    Req_info &ri = req_info()[i];
    if (ri.lv >= view() || ri.v >= view()) return false;
  }
...
bool View_change::verify_digest() {
  ...
  bool verified =
      (d == Digest(contents(),
                   sizeof(View_change_rep) + sizeof(Req_info) * rep().n_reqs));
...
bool View_change::convert(Message *m1, View_change *&m2) {
  if (!m1->has_tag(View_change_tag, sizeof(View_change_rep))) return false;
