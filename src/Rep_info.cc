...
Rep_info::~Rep_info() {
  for (int i = 0; i < nps; i++) delete reps[i];
}
...
char* Rep_info::new_reply(int pid, int& sz) {
  ...
  for (int i = (r->contents() - mem) / Block_size;
    ...
    //    fprintf(stderr,"modifying reply: mem %d begin %d end %d \t page %d
    //    size %d\n",mem, r->contents(), r->contents()+Max_rep_size-1,
    //    i,size());
...
bool Rep_info::new_state(Req_queue* rset) {
  ...
  for (int i = 0; i < nps; i++) {
    ...
    if (rset->remove(i, req_id(i))) first = true;
  }
  return first;
}
