...
void Statistics::zero_stats() {
  ...
  if (ret != 0) th_fail("getrusage failed");
...
void Statistics::print_stats() {
  ...
  printf("Generate = %qd (cycles) %qd (max. op cycles) ops= %ld \n",
         gen_auth_cycles.elapsed(), gen_auth_cycles.max_increment(),
         num_gen_auth);

  printf("Verify = %qd (cycles) %qd (max. op cycles) ops= %ld \n",
         ver_auth_cycles.elapsed(), ver_auth_cycles.max_increment(),
         num_ver_auth);

  printf("\nReply MAC stats:\n");
  printf("Generate = %qd (cycles) %qd (max. op cycles) ops= %ld \n",
         reply_auth_cycles.elapsed(), reply_auth_cycles.max_increment(),
         reply_auth);

  printf("Verify = %qd (cycles) %qd (max. op cycles) ops= %ld \n",
         reply_auth_ver_cycles.elapsed(), reply_auth_ver_cycles.max_increment(),
         reply_auth_ver);
  ...
  printf("Generate = %qd (cycles) %qd (max. op cycles) ops= %ld \n",
         pp_digest_cycles.elapsed(), pp_digest_cycles.max_increment(),
         pp_digest);

  printf("\nRecovery stats:\n");
  printf("Recoveries = %ld overlaps = %ld incompletes = %ld\n", rec_counter,
         rec_overlaps, incomplete_recs);
  ...
  printf("Rollbacking = %qd (cycles) %qd (max. op cycles) ops= %ld \n",
         rollback_cycles.elapsed(), rollback_cycles.max_increment(),
         num_rollbacks);
  ...
  printf("Recvfrom = %qd (cycles) %qd (max. op cycles) ops= %ld success= %ld\n",
         recvfrom_cycles.elapsed(), recvfrom_cycles.max_increment(),
         num_recvfrom, num_recv_success);
  ...
  printf(
      ...
      select_cycles.elapsed(), select_cycles.max_increment(), select_success,
      select_fail);
...
Recovery_stats::Recovery_stats() { zero_stats(); }
