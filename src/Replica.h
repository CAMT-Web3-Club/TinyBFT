...
class Replica : public Node {
 ...
 private:
  ...
  //! can be outstanding at any one time. ie. with CW=1, can't send a pp unless
  ...
  //
  ITimer
      *rtimer;  // Recovery timer. TODO: Timeout should be generated by watchdog
...
inline int Replica::used_state_bytes() const { return replies.size(); }
