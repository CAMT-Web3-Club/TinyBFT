...
bool New_view::view_change(int id, Digest &d) {
  if (id < 0 || id >= node->n()) return false;
...
bool New_view::verify() {
  ...
  for (Seqno i = min(); i < max(); i++) {
    ...
    if (!node->is_replica(vci) || vc_info()[vci].d.is_zero()) return false;
  }
...
bool New_view::convert(Message *m1, New_view *&m2) {
  if (!m1->has_tag(New_view_tag, sizeof(New_view_rep))) return false;
