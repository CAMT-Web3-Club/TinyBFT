...
Reply::Reply(View view, Request_id req, int replica, Digest &d, Principal *p,
             bool tentative)
...
bool Reply::verify() {
  ...
  if (size() - (int)sizeof(Reply_rep) - rep_size < MAC_size) return false;

  // Check reply
  if (full()) {
    Digest d(contents() + sizeof(Reply_rep), rep_size);
    if (d != rep().digest) return false;
  }
  ...
  bool ret = replica->verify_mac_in(contents(), sizeof(Reply_rep),
...
bool Reply::convert(Message *m1, Reply *&m2) {
  if (!m1->has_tag(Reply_tag, sizeof(Reply_rep))) return false;
