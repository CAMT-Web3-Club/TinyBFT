...
Page_mapping::~Page_mapping() { clear(); }
...
inline void Page_mapping::append_or_replace(int l, int i, BlockCopy* b) {
  BlockCopy* tmp;
  if (parts.remove(PartKey(l, i), tmp)) delete tmp;
  append(l, i, b);
}

inline void Page_mapping::append_or_replace_without_delete(int l, int i,
                                                           BlockCopy* b) {
  BlockCopy* tmp;
...
inline int Page_mapping::num_entries() const { return parts.size(); }
...
inline int PageCache::num_entries() const { return parts.size(); }
...
State::State(Replica* rep, int total_pages,
#ifdef OBJ_REP
             int (*gets)(int, char**), void (*puts)(int, int*, int*, char**),
             void (*shutdown_p)(FILE* o), void (*restart_p)(FILE* i)
#else
             void (*getp)(int, char*), void (*putp)(int, int*, char**)
#endif
#ifdef APP_CHKPTS
                                              ,
             void (*take_cp)(int), void (*discard_cps)(int),
    ...
    : replica(rep),
      ...
      ,
      take_chkpt(take_cp),
      discard_chkpts(discard_cps),
      get_page_from_chkpt(get_page_from_cp),
      get_digest_from_chkpt(get_digest_from_cp)
#endif
{
...
#ifdef OBJ_REP
      char* data;
      //      fprintf(stderr, "!@$#\%%&^^*& Calling get_segm on
      //      cow!!!!!!!!!!!\n");
...
inline int State::digest(Digest& d, int l, int i) {
  ...
  if (l == PLevels - 1) {
    ...
    if (i < nb - replica->used_state_pages()) {  // application state
      ...
      //      if (!checking) {
      if (pages_lc[i]) delete pages_lc[i];
      pages_lc[i] = bcp;
      //      }
...
#ifdef APP_CHKPTS
    if (!b->data_present) get_page_from_chkpt(lc, i, (char*)(&(b->data)));
...
char* State::get_data(Seqno c, int i) {
  ...
  if (ptree[PLevels - 1][i].lm <= c && !cowb.test(i)) {
    ...
    if (i < nb - replica->used_state_pages()) {  // application state
      ...
      //      fprintf(stderr, "Replacing Obj %d, c=%d for (i,c) = (%d,%d)
      //      \t",last_i, last_c, i, c);
      if (last_obj) delete[] last_obj;
...
void State::start_fetch(Seqno le, Seqno c, Digest* cd, bool stable) {
  ...
  if (!fetching) {
#ifndef NO_STATE_TRANSLATION
    th_assert(fetched_pages->num_entries() == 0,
              "Fetched Pages map not cleared");
#endif
    INCR_OP(num_fetches);

    fetching = true;
    // XXXXXXit should be like this  keep_ckpts = lc >= 0; see if I can avoid
    // keeping
    // checkpoints if I am fetching a lot of stuff I can allocate more memory
...
void State::send_fetch(bool change_replier) {
  ...
  //  fprintf(stderr, "Sending fetch message: rid=%qd lu=%qd (%d,%d) c=%qd
...
bool State::handle(Fetch* m, Seqno ls) {
  ...
  if (m->verify()) {
    ...
    if (pi->last_fetch_rid() < m->request_id() && (l < PLevels - 1 || i < nb)) {
      ...
      //      fprintf(stderr, "Rx FTCH. ls=%qd rc=%qd lu=%qd lm=%qd.
      //      ",ls,rc,m->last_uptodate(),ptree[l][i].lm);
        ...
        if (chosen >= 0) {
          if (l == PLevels - 1) {
            ...
            for (; j < max; j++) {
              ...
              if (q.lm > thr) md.add_sub_part(j, q.d);
            }
...
void State::handle(Data* m) {
  ...
  if (fetching && flevel == l) {
    ...
    int i = wp.index;
    //    fprintf(stderr, "RxDat i=%d(sthi%d) chnkn %d,tot-sz %d,nchnks
    //    %d#total_sz %d,next_chnk %d\n", m->index(), i,m->chunk_number(),
    //    m->total_size(),m->num_chunks(),total_size, next_chunk);
    if (m->index() == i) {
#ifdef OBJ_REP
      if (m->chunk_number() < m->num_chunks() &&
        ...
        if (m->chunk_number() <
            m->num_chunks() - 1) {  // Not final chunk - perform reassembly and
                                    // send another fetch for the next fragment
          if (next_chunk == 0) {    // First chunk - allocate buffer
            th_assert(reassemb == NULL && m->total_size() > Fragment_size,
                      "Unexpected reassembly buffer");
            ...
            th_assert(reassemb != NULL &&
                          m->total_size() > (next_chunk + 1) * Fragment_size,
                      "Unexpected reassembly buffer");
      ...
      digest(d, i, m->last_mod(), m->data(), Block_size);
        ...
        if (i < nb - replica->used_state_pages()) {  // application state
          ...
          bc->data =
              m->data();  // XXXX RR-TODO: (optimize) get rid of this copying!!!
#endif
          ...
          rep_mem[i - nb + replica->used_state_pages()] = m->data();
            ...
            th_assert(pwp.index == i / PSize[l],
                      "Parent is not first at level l-1 queue");
            if (p.lm > pwp.lm) {
              pwp.lm = p.lm;
            }
...
void State::handle(Meta_data* m) {
  ...
  if (fetching && flevel < PLevels - 1 && m->request_id() == crid &&
      flevel == m->level()) {
    ...
    if (wp.index == m->index() && wp.c >= 0 && m->digest() == wp.d) {
      ...
      if (m->verify() && check_digest(wp.d, m)) {
        ...
        while (iter.get(index, d)) {
          if (flevel == PLevels - 1 && index >= nb) break;
...
void State::handle(Meta_data_d* m) {
  ...
  if (fetching && m->request_id() == crid && flevel == m->level()) {
    ...
    if (wp.index == m->index() && m->last_stable() >= lc &&
        m->last_stable() >= wp.lu) {
...
void State::done_with_level() {
  ...
  if (wp.lu >= wp.lm) {
    ...
    if (l == 0) {
      // Completed fetch
      fetching = false;
      if (checking && to_check->size() == 0) checking = false;
      ...
      // Moving to a sibling.
      if (flevel <= refetch_level) refetch_level = PLevels;
    }
  } else {
    // Partition is inconsistent
    if (flevel < refetch_level) refetch_level = flevel;
  }
...
void State::mark_stale() { cert->clear(); }
...
#ifdef OBJ_REP
          if (l == PLevels - 1) {
            //	    fprintf(stderr, "Wrt i=%d, sz=%d \t",i, ((BlockCopy
            //*)p)->data.size);
            wb += fwrite(((BlockCopy*)p)->data.data, ((BlockCopy*)p)->data.size,
                         1, o);
            ab++;
          }
#endif
...
bool State::restart(FILE* in, Replica* rep, Seqno ls, Seqno le, bool corrupt) {
  ...
  for (int l = PLevels - 1; l > 0; l--) {
    for (int i = 0; i < np; i++) {
      if (i % PSize[l] == 0) stree[l - 1][i / PSize[l]] = DSum();
  ...
  Digest d;
  digest(d, 0, 0);
  if (d != ptree[0][0].d) ret = false;

  clog.clear(ls);
  rb += fread(&lc, sizeof(Seqno), 1, in);
  if (lc < ls || lc > le) return false;
...
#ifdef OBJ_REP
      if (l == PLevels - 1) {
        //	fprintf(stderr, "Rd i=%d, sz=%d \t",i, ((BlockCopy
        //*)p)->data.size);
