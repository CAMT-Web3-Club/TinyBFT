...
class NV_info {
 ...
 private:
  ...
  Array<Array<Req_sum> >
      reqs;              // reqs contains an array for each sequence number
  ...
  int n_complete;  // Number of complete entries with seqnos between min and
                   // max.
...
inline View NV_info::view() const { return v; }

inline bool NV_info::complete() const { return is_complete; }
