...
#else
  if (size() - (int)sizeof(Checkpoint_rep) < node->sig_size(id())) return false;

  return node->i_to_p(id())->verify_signature(
      contents(), sizeof(Checkpoint_rep), contents() + sizeof(Checkpoint_rep));
#endif
}

bool Checkpoint::convert(Message *m1, Checkpoint *&m2) {
  if (!m1->has_tag(Checkpoint_tag, sizeof(Checkpoint_rep))) return false;
  m1->trim();
  m2 = (Checkpoint *)m1;
  return true;
}
