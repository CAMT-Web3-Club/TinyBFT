...
bool Meta_data::verify() {
  ...
  if (!node->is_replica(id()) || node->id() == id()) return false;

  // Meta_data messages are not sent for leaf partitions
  if (level() < 0 || level() >= PLevels - 1) return false;

  if (index() < 0 || index() >= PLevelSize[level()]) return false;

  if (rep().np > 1 && rep().np > PSize[level() + 1]) return false;

  if (last_mod() > last_uptodate()) return false;
...
bool Meta_data::convert(Message *m1, Meta_data *&m2) {
  if (!m1->has_tag(Meta_data_tag, sizeof(Meta_data_rep))) return false;
  m1->trim();
  m2 = (Meta_data *)m1;
  return true;
}
