#ifndef LIBBYZEA_MODIFY_H
#define LIBBYZEA_MODIFY_H
...
#define _Byz_cow_bit(bindex) \
  (_Byz_cow_bits[bindex / _LONGBITS] & (1UL << (bindex % _LONGBITS)))

#define _Byz_modify1(mem)                                     \
  do {                                                        \
    int bindex;                                               \
    bindex = ((char *)(mem)-_Byz_mem) / Block_size;           \
    if (_Byz_cow_bit(bindex) == 0) _Byz_modify_index(bindex); \
  } while (0)

#define _Byz_modify2(mem, size)                                      \
  do {                                                               \
    int bindex1;                                                     \
    int bindex2;                                                     \
    char *_mem;                                                      \
